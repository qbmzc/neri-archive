-- 已发布的 0.1.0 版 schema.sql 原样副本，用作升级回归测试的输入。
-- 不要随着版本更新这份文件：它代表「用户手上最老的库」，一旦跟着更新就失去了保护作用
-- （见 SchemaMigrationsTest.upgradesAReleasedDatabaseWithoutFailingOnSchemaSql）。
