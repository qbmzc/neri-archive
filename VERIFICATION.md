...
## 2026-09-16 深色模式与重复文件一键清理

环境：macOS、Oracle GraalVM JDK 21.0.8、Maven 3.9.12、Node 26.8.2。

- 74 项离线自动化测试，0 失败、0 错误、1 跳过（LiveGatewayTest 需联网开关），本次新增 7 项：
  - 5 项重复清理：按音频指纹时长与字节估算的码率高者保留、其余进回收站；归档文件不是保留对象时先改指再移动；内容完全相同时优先保留归档文件；同一音频属于两首已归档歌曲时整组跳过；文件在扫描后发生变化时跳过。
  - 1 项回收站：统计文件数与占用；`emptyTrash` 删除全部文件并回收空目录。
  - 1 项存储：改指歌曲后存量表 `song_id` 同步，清理后不足两个文件的分组解散。
- Vue TypeScript 检查与 Vite 生产构建通过。
- 深色模式：颜色全部收敛为 CSS 变量，`[data-theme=dark]` 整体覆盖；默认跟随系统，右上角手动切换并写入 localStorage，`index.html` 内联脚本避免首屏闪白。用无头 Chromium 对构建产物截图核对浅色/深色两套主题（总览、音乐档案、表格、状态徽章、弹窗、提示条、回收站条），浅色与原版一致；深色关键文本对比度均不低于 4.5:1。
- 重复清理接口：`POST /api/library/duplicates/cleanup`、`GET /api/library/trash`、`POST /api/library/trash/empty`。

未验证：未在真实浏览器中使用管理员会话完整点击「清理重复副本 → 清空回收站」（后端行为由自动化测试覆盖，按钮与回收站条仅做静态渲染核对）；未在跨文件系统的回收站（`ARCHIVE_TRASH` 指向另一块盘）上执行清理。
