...
- 独立管理员登录，CSRF 防护，网易云 Cookie 导入验证、扫码登录入口及登录失效处理。界面提供深色模式，默认跟随系统，可在右上角切换并记住选择。
...
- 启动后清点音乐目录中的现有音频：SHA-256 确认文件完全相同，Chromaprint 音频指纹识别标签、容器或编码不同但声音内容相近的疑似副本；支持手动重扫、搜索及分页查看全部存量文件。一键清理按「码率优先、体积其次」每组保留一份，其余移入回收站，不会直接删除。
...
# 或指定版本镜像 ghcr.io/qbmzc/neri-archive:0.2.2
```
...
java -jar target/neri-archive-0.2.2.jar
```
...
- 回收站位于 `<ARCHIVE_MUSIC>/.trash/<歌曲ID>/`，文件名带时间戳前缀，按修改时间到期清理；它不参与扫描，也不出现在音乐档案页。恢复方式是把文件移回原目录；音乐档案页显示回收站文件数与占用，可一键清空（清空前二次确认，清空后无法恢复）。
- 一键清理重复只移动未被归档引用的副本；同一音频属于多首已归档歌曲时整组跳过，避免破坏任何一首歌的归档路径。文件自扫描后发生变化时跳过对应分组，清理前会先校验文件大小与修改时间。
